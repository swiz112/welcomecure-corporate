...
test('Email column should display masked values (e.g., **********) in UI', async ({ page }) => {
  ...
  expect(count).toBeGreaterThan(0, 'No email cells found in table');
...
test('Contact Number column should display masked values (e.g., **********) in UI', async ({ page }) => {
  ...
  const count = await emailCells.count();
  expect(count).toBeGreaterThan(0, 'No contact number cells found in table');
